...
namespace PBDFluid
{
    ...
    public class PBDFluidSolver : FluidSolver {

        public int SolverIterations { get; set; }

        public int ConstraintIterations { get; set; }

        public PBDFluidSolver(FluidBody body, FluidBoundary boundary) : base(body, boundary) {
            SolverIterations = 1;
            ConstraintIterations = 2;

            m_shader = Resources.Load("ComputeShaders/PBDSolver") as ComputeShader;
        }
        ...
        public override void StepPhysics(float dt)
        {
            ...
            m_shader.SetFloat("BoundaryPSI", Boundary.Density / Kernel.Poly6(Vector3.zero));
